<x-layouts.lms title="Create Lesson">
    <h1 class="font-serif text-3xl font-semibold text-zinc-900">Create Lesson</h1>

    <form method="POST" action="{{ route('lms.admin.lessons.store') }}" class="mt-8 space-y-5 rounded-xl border border-zinc-200 bg-white p-6">
        @csrf
        ...
        <div>
            ...
            <textarea id="summary" name="summary" rows="3" class="mt-1 w-full rounded-md border border-zinc-300 px-3 py-2 text-sm">{{ old('summary') }}</textarea>
        ...
        <div>
            ...
            <textarea id="content" name="content" rows="6" class="mt-1 w-full rounded-md border border-zinc-300 px-3 py-2 text-sm">{{ old('content') }}</textarea>
        ...
        <div>
            ...
            <input id="position" name="position" type="number" min="1" value="{{ old('position', 1) }}" class="mt-1 w-full rounded-md border border-zinc-300 px-3 py-2 text-sm">
            @error('position')<p class="mt-1 text-sm text-red-700">{{ $message }}</p>@enderror
        </div>

        <div class="flex items-center gap-2">
            <input id="is_published" name="is_published" type="checkbox" value="1" @checked((bool) old('is_published'))>
        ...
        <button type="submit" class="rounded-md bg-sky-700 px-4 py-2 text-sm font-semibold text-white hover:bg-sky-800">Create lesson</button>
    </form>
</x-layouts.lms>
